...
<head>
    ...
    <title>{{lcfirst(env('APP_NAME','Laravel'))}}</title>
    ...
    <div class="columns-3 gap-3">
        ...
        <div>
            <select id="hostAcc"
                ...
                @foreach($hostAcc as $user)
                    <option value="{{$user}}" {{$user===$currentUser? 'selected': ''}}>{{$user}}</option>
                @endforeach
            </select>
        </div>
        <div>
                ...
                <div class="p-6 space-y-6">
                    <form action="{{route('addSite')}}" method="post">
                        ...
                        <div class="grid mb-6 md:grid-cols-1">
                            ...
                            <div class="mb-6">
                                <label for="dbPass"
                                       class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Пароль
                                    для базы</label>
